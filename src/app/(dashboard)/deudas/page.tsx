...
type Debt = { id: string; name: string; total_amount: number; remaining_amount: number; monthly_payment: number; due_date: string; type: string }
  ...
  return (
    <div className="space-y-6">
      <div className="bg-red-500/10 border border-red-500/20 rounded-2xl p-5 flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-white text-2xl font-bold">RD$ {totalDebt.toLocaleString('es-DO', { minimumFractionDigits: 2 })}</p>
          </div>
        </div>
        <button onClick={() => setShowForm(true)} className="flex items-center gap-2 bg-red-500 hover:bg-red-600 text-white px-3 py-1.5 rounded-lg text-sm transition-colors">
      ...
      <div className="space-y-3">
        {debts.length === 0 ? (
          <div className="bg-white/5 border border-white/5 rounded-2xl p-8 text-center text-slate-500 text-sm">No tienes deudas registradas.</div>
        ) : debts.map(d => {
          const pct = Math.round(((Number(d.total_amount) - Number(d.remaining_amount)) / Number(d.total_amount)) * 100)
          const badge = pct >= 80 ? { label: 'Casi pagado', color: 'emerald' } : pct >= 40 ? { label: 'En progreso', color: 'yellow' } : { label: 'Pendiente', color: 'red' }
          return (
            <div key={d.id} className="bg-white/5 border border-white/5 rounded-2xl p-5">
              <div className="flex justify-between items-start mb-3">
                <div>
                  ...
                  <p className="text-slate-500 text-xs">Pago mensual: RD$ {Number(d.monthly_payment).toLocaleString('es-DO')}</p>
                </div>
                <span className="text-red-400 font-semibold text-sm">RD$ {Number(d.remaining_amount).toLocaleString('es-DO')}</span>
              </div>
              <div className="w-full bg-white/10 rounded-full h-1.5 mb-1">
                <div className="bg-emerald-500 h-1.5 rounded-full" style={{ width: `${pct}%` }} />
              </div>
              <p className="text-slate-500 text-xs">{pct}% pagado · Total: RD$ {Number(d.total_amount).toLocaleString('es-DO')}</p>
            </div>
          )
        })}
        ...
        <button onClick={() => setShowForm(true)} className="w-full bg-white/5 border-2 border-dashed border-white/10 rounded-2xl p-4 flex items-center justify-center gap-2 text-slate-500 hover:text-emerald-400 hover:border-emerald-500/40 transition-colors text-sm">
          <Plus size={18} /> Registrar nueva deuda
        </button>
      </div>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-[#0f1923] border border-white/10 rounded-2xl p-6 w-full max-w-sm">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-white font-semibold">Nueva deuda</h3>
            ...
            <form onSubmit={addDebt} className="space-y-3">
              <input value={name} onChange={e => setName(e.target.value)} required placeholder="Nombre (ej: Tarjeta Popular)"
